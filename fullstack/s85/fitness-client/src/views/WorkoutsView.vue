<template>
  ...
  >
    <div class="hero-content">
      ...
      <div class="d-flex gap-2 mt-4 flex-wrap">
        ...
        <!-- changed from btn-outline-light to btn-outline-dark -->
        <button class="btn btn-outline-dark" @click="workouts.fetchMyWorkouts()">
          Refresh
        </button>
      </div>
    </div>
  ...
  <div class="row g-2 mb-3">
    <div class="col-12 col-md-6">
      <input v-model="search" class="form-control" placeholder="Search workout name..." />
    </div>
  ...
  <div v-else class="row g-3">
    ...
    <div v-for="w in visibleWorkouts" :key="w._id" class="col-12 col-md-6 col-lg-4">
      <WorkoutCard :workout="w" @complete="workouts.completeWorkout" @delete="workouts.deleteWorkout" />
    </div>
  </div>
        ...
        <div class="modal-body">
          <form @submit.prevent="submitAdd">
            ...
            <div class="mb-2">
              ...
              <input v-model="duration" class="form-control" placeholder="e.g. 45 minutes" required />

              <!-- changed from text-light opacity-75 -->
              <div class="form-text small-muted">
                Duration is stored as text (example: “45 minutes”).
              </div>
            </div>

            <div class="d-flex gap-2 mt-3">
              <button class="btn btn-accent w-100" type="submit">Add</button>
              <button class="btn btn-outline-dark w-100" type="button" data-bs-dismiss="modal">
                Cancel
              </button>
            </div>
          </form>
